...
public:
    ...
    {
        // 模型初始化
        this->predictor_nna_ = std::make_shared<PPNCPredictor>(pathModel + "/config_ppncnna.json");
        this->predictor_nms_ = std::make_shared<PPNCPredictor>(pathModel + "/config_ppncnms.json");
    ...
    void buildNms(const std::string &model_dir)
    {
        ...
        std::string untar_cmd = "tar -xf " + model_file + " -C " + model_dir + " --no-same-owner";
        std::string final_file = model_file + ".so";
        std::string cc_cmd = "g++ -shared -fPIC -o " + final_file + " " + model_dir + "/lib0.o " + model_dir + "/devc.o";
        int sys_status = 0;
