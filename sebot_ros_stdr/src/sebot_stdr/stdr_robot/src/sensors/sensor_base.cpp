 ...
 **/
  ...
  **/
  void Sensor::updateTransform(const ros::TimerEvent &)
  {
    try
    {
      _tfListener.waitForTransform("map_static", _sensorFrameId, ros::Time(0), ros::Duration(0.2));
      _tfListener.lookupTransform("map_static", _sensorFrameId, ros::Time(0), _sensorTransform);
      _gotTransform = true;
    }
