...
int main(int argc, char **argv)
{
  ...
  if (((argc == 3) || (argc == 6)) && (std::string(argv[1]) == "add"))
  {
    ...
    try
    {
      msg = stdr_parser::Parser::createMessage<stdr_msgs::RobotMsg>(std::string(argv[2])); // 加载机器人配置
    }
    ...
    if (argc == 6)
    {
      msg.initialPose.x = atof(argv[3]);     // 1
      msg.initialPose.y = atof(argv[4]);     // 2
      msg.initialPose.theta = atof(argv[5]); // 0
    }
