...
namespace stdr_gui
{
  ...
  @return void
  **/
  CGuiLaser::CGuiLaser(stdr_msgs::LaserSensorMsg msg, std::string baseTopic) : msg_(msg)
  {
    topic_ = "/scan";
    tf_frame_ = msg_.frame_id;
    ros::NodeHandle n;
    lock_ = false;
    ...
    for (unsigned int i = 0; i < scan_.ranges.size(); i++)
    {
      ...
      painter.drawLine(
          ...
          pose_x / ocgd + real_dist *
                              cos(pose_theta + scan_.angle_min + i * scan_.angle_increment) / ocgd,

          pose_y / ocgd + real_dist *
                              sin(pose_theta + scan_.angle_min + i * scan_.angle_increment) / ocgd);
    }
    lock_ = false;
  }
    ...
    for (unsigned int i = 0; i < scan_.ranges.size(); i++)
    {
      ...
      painter.drawLine(
          ...
          size / 2 + ((msg_.pose.x / ocgd) + real_dist *
                                                 cos(scan_.angle_min + i * scan_.angle_increment + msg_.pose.theta) / ocgd) *
                         climax,
          size / 2 + ((msg_.pose.y / ocgd) + real_dist *
                                                 sin(scan_.angle_min + i * scan_.angle_increment + msg_.pose.theta) / ocgd) *
                         climax);
    }
    lock_ = false;
  }
