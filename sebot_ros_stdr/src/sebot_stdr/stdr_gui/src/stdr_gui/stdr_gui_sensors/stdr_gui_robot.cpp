...
namespace stdr_gui
{
  ...
  **/
  void CGuiRobot::draw(QImage *m, float ocgd, tf::TransformListener *listener)
  {
    ...
    {
      ros::NodeHandle n_;
      std::string speeds_topic = "/cmd_vel";
  ...
  void CGuiRobot::drawSelf(QImage *m)
  {
    ...
    else
    {
      ...
      for (unsigned int i = 0; i < footprint_.points.size() + 1; i++)
      {
        ...
        points[i] = QPointF(
            current_pose_.x / resolution_ +
                x / resolution_ * cos(-current_pose_.theta) + y / resolution_ * sin(-current_pose_.theta),

            current_pose_.y / resolution_ +
                x / resolution_ * sin(current_pose_.theta) + y / resolution_ * cos(-current_pose_.theta));
